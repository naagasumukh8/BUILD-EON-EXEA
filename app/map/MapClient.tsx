...
function getDestConfig(destName: string): DestConfig {
  ...
  if (lower.includes('japan') || lower.includes('tokyo') || lower.includes('yokohama')) {
  ...
  if (lower.includes('china') || lower.includes('shanghai') || lower.includes('ningbo') || lower.includes('qingdao') || lower.includes('beijing')) {
  ...
  if (lower.includes('rotterdam') || lower.includes('europe') || lower.includes('netherlands') || lower.includes('antwerp') || lower.includes('hamburg')) {
  ...
  if (lower.includes('houston') || lower.includes('usa') || lower.includes('america')) {
    return { center: [28.0, -50.0], zoom: 3, destCoords: [29.76, -95.36], destLabel: 'Houston Hub, USA', showIpsaPipeline: true, showHormuz: false }
  }
  if (lower.includes('uk') || lower.includes('london') || lower.includes('britain')) {
    return { center: [40.0, 0.0], zoom: 3, destCoords: [51.50, 0.12], destLabel: 'London Port, UK', showIpsaPipeline: true, showHormuz: true }
  }
  if (lower.includes('sydney') || lower.includes('australia')) {
    return { center: [-10.0, 110.0], zoom: 3, destCoords: [-33.86, 151.20], destLabel: 'Sydney Port, Australia', showIpsaPipeline: false, showHormuz: true }
  }
...
function MapContent() {
  ...
  const keyPorts = [
    { key: 'origin', lat: SUPPLY_ORIGIN_COORDS[0], lon: SUPPLY_ORIGIN_COORDS[1], name: 'Ras Tanura Terminal — Supply Origin', color: '#7c3aed', emoji: '🛢️' },
        ...
        <div className="absolute inset-0 z-0">
          {!loading && typeof window !== 'undefined' && (
            <MapContainer center={destCfg.center} zoom={destCfg.zoom} scrollWheelZoom className="w-full h-full">
              ...
              {showHormuzMarker && (
                <Marker position={HORMUZ_COORDS} icon={createIcon(L, '#DC2626', '⚠️', 30)}>
                  <Popup>
